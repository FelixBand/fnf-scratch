...
def process_sprite_sheet(xml_file_path, image_path, output_folder):
    ...
    for sub_texture in root.findall('.//SubTexture'):
        ...
        if current_animation != animation_name:
            offset_x = int(input(f"Enter X offset for animation '{animation_name}': "))
            offset_y = int(input(f"Enter Y offset for animation '{animation_name}': "))

            # Update the current animation
            current_animation = animation_name

        # Apply user defined offsets
        frame_width += offset_x
        frame_height += offset_y
